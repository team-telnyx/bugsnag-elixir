defmodule BugsnagTest do
  ...
  test "it puts application env values on startup" do
    ...
    assert Application.get_env(:bugsnag, :ignored_exceptions) == []
  end
  ...
  test "does not notify bugsnag if exception is in ignored_exceptions" do
    old_ignore_exceptions = Application.get_env(:bugsnag, :ignored_exceptions)
    Application.put_env(:bugsnag, :ignored_exceptions, [RuntimeError])
    on_exit(fn -> Application.put_env(:bugsnag, :ignored_exceptions, old_ignore_exceptions) end)
